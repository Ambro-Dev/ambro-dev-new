...
export const EnhancedButton: FC<{
  ...
  ({
    ...
    // Create a mutable ref instead of readonly
    const buttonRef = useRef<HTMLButtonElement | HTMLAnchorElement | null>(
      null
    );

    // Fixed merged ref function using a proper pattern
    const mergedRef = useCallback(
      (node: HTMLButtonElement | HTMLAnchorElement | null) => {
        // Set internal ref
        buttonRef.current = node;

        // Forward to external ref if it exists
        if (ref) {
          // Handle the external ref safely by type assertion
          (
